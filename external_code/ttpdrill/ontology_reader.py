...
class ReadOntology:
    ...
    def read_mitre_TTP(self, file_name="resources/mitre_ttp.csv"):
        """
        Creates a dictionary containing for each mitre_id a dictionary value containing informations like:
        - tecnique name,
        - tactic description

        Returns:
        dict: dictonary of type {mitre_id -> {tecnique: .., tactic: ...}}
        """
        ...
        for mitre_tech in self.mitre_tech_list:
            try:
                ...
                tactic = ""
            self.mitre_tech_dict[mitre_tech["ID"].lower()] = {
                "TECHNIQUE": mitre_tech["TECHNIQUE"],
                "TACTIC": tactic + mitre_tech["TACTIC"],
            }
        return self.mitre_tech_dict

    def read_csv(self, file_name):
        """
        Open file containing already built ontology
        """
        if file_name is None:
            self.data_frame = pandas.read_csv(
                "resources/ontology_details.csv", encoding="ISO-8859-1"
            )
        else:
    ...
    def split_ontology_list(self, ontology_list):
        """
        For the action_what, action_where, why_what, why_whwer, how_what, how_where entry in the ontologies
    ...
    def write_ontology(self, file_name, data_frame):
        ...
        if file_name is not None:
            data_frame.to_csv(
                r"resources/export_dataframe_1.csv", index=None, header=True
            )
        else:
    ...
    def parse_ontology(self, data_frame_dict):
        ...
        for dict_entry in enumerate(data_frame_dict):
            ...
            for key, val in dict_entry[1].items():
                try:
                    if key == "CODE":
                        ontology_dict_map[dict_entry[0]] = (
                            re.search("T\d*", val).group().lower()
                        )
                    if key == "NAME":
...
def split_1(ontology_dict):
    ...
    for __each__entry__ in ontology_dict:
        ...
        list_.append(
            [
                __each__entry__["Id"],
                __each__entry__["action_what"].split("/"),
                __each__entry__["action_where"],
            ]
        )
