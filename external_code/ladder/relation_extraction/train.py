...
def flat_accuracy(preds, labels):
    ...
    return np.sum(pred_flat[non_zero_idx] == labels_flat[non_zero_idx]) / len(
        labels_flat[non_zero_idx]
    )
    ...
    if NUM_LABELS == 4:
        print(
            classification_report(
                ...
                prediction,
                labels=[0, 1, 2, 3],
                target_names=["has", "uses", "targets", "indicatedBy"],
            )
        )
    elif NUM_LABELS == 6:
        print(
...
def main(argv=None):
    ...
    sentence_train_label = json.load(
        open("data/" + DATASET + "/train_label_id.json", "r")
    )
    train_dataset = pre_processing(sentence_train, sentence_train_label)

    # split training data to labeled set and unlabeled set
    labeled_dataset, unlabeled_dataset_total = stratified_sample(
        train_dataset, LABEL_OF_TRAIN
    )

    unlabeled_dataset = []
    for i in range(MATE_EPOCHS):
        unlabeled_dataset_now, unlabeled_dataset_total = stratified_sample(
            unlabeled_dataset_total, UNLABEL_OF_TRAIN / MATE_EPOCHS
        )
    ...
    scheduler1 = get_linear_schedule_with_warmup(
        optimizer1, num_warmup_steps=0, num_training_steps=total_steps1
    )
    scheduler2 = get_linear_schedule_with_warmup(
        optimizer2, num_warmup_steps=0, num_training_steps=total_steps2
    )
    ...
    for total_epoch in range(TOTAL_EPOCHS):
        ...
        for mate_epoch in range(MATE_EPOCHS):
            ...
            for epoch_i in range(0, EPOCHS):
                ...
                for batch in validation_dataloader:
                    ...
                    all_prediction = np.concatenate(
                        (all_prediction, pred_flat), axis=None
                    )
            ...
            all_pseudo_prediction = np.concatenate(
                (all_pseudo_prediction, pred_flat), axis=None
            )
            all_pseudo_ground_truth = np.concatenate(
                (all_pseudo_ground_truth, labels_flat), axis=None
            )
            all_pseudo_weights = np.concatenate(
                (all_pseudo_weights, weights_flat), axis=None
            )
            ...
            score(all_ground_truth, all_prediction)
            np.save(
                LOG_DIR + "/" + LOG_DIR + "_all_prediction{}".format(cnt),
                all_prediction[np_sort],
            )
            np.save(
                LOG_DIR + "/" + LOG_DIR + "_all_label{}".format(cnt),
                all_ground_truth[np_sort],
            )
            np.save(
                LOG_DIR + "/" + LOG_DIR + "_all_weights{}".format(cnt),
                all_weights[np_sort],
            )
            #  evaluate pseudo label quality
            ...
            np.save(
                LOG_DIR + "/" + LOG_DIR + "_all_pseudo_prediction{}".format(cnt),
                all_pseudo_prediction,
            )
            np.save(
                LOG_DIR + "/" + LOG_DIR + "_all_pseudo_label{}".format(cnt),
                all_pseudo_ground_truth,
            )
            np.save(
                LOG_DIR + "/" + LOG_DIR + "_all_pseudo_weights{}".format(cnt),
                all_pseudo_weights,
            )
            cnt += 1

            # update training data
            train_add_dataset = train_dataloader.dataset + TensorDataset(
                input_ids, input_mask, pseudo_labels, e1_pos, e2_pos, w
            )
        ...
        validation_acc = np.sum(
            all_prediction[non_zero_idx] == all_ground_truth[non_zero_idx]
        ) / len(all_ground_truth[non_zero_idx])
        validation_f1_score = f1_score(
            all_ground_truth[non_zero_idx],
            all_prediction[non_zero_idx],
            average="micro",
        )
