...
class EntityRecognitionDataset(torch.utils.data.Dataset):
    def __init__(self, files, tokenizer, sequence_len, token_style):
        ...
        if isinstance(files, list):
            self.data = []
            for file in files:
                self.data += parse_entity_data(
                    file, tokenizer, sequence_len, token_style
                )
        else:
...
class SentenceClassificationDatasetBERT(torch.utils.data.Dataset):
    """
    Custom Dataset used for the sentence classification task using BERT
    """
    ...
    def __init__(self, file_path, sequence_len, bert_model, num_class=2, balance=False):
        ...
        if balance:
            ...
            self.data = _data
        tokenizer = MODELS[bert_model][1]  # (AutoModel, AutoTokenizer, 768, 'roberta'),
        self.tokenizer = tokenizer.from_pretrained(bert_model)
        self.sequence_len = sequence_len
        token_style = MODELS[bert_model][
            3
    ...
    def __len__(self):
        """
        Returns the numbers of examples in the dataset
        """
        return len(self.data)
    ...
    def __getitem__(self, index):
        ...
        Parameters:
        index (int): index of the element to take from the dataset

        Returns:
        list, list, int, str: token ids, attention mask, label, text to predict
        """
        ...
        if len(tokens) < self.sequence_len:
            tokens = tokens + [
                self.pad_token for _ in range(self.sequence_len - len(tokens))
            ]
        else:
