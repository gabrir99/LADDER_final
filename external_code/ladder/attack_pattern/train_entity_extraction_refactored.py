...
class EntityClassification:
    ...
    def __init__(self, args):
        ...
        # tokenizer
        self.tokenizer = MODELS[self.args.pretrained_model][1].from_pretrained(
        ...
        train_set = Dataset(
            os.path.join(self.args.dataset, "train.txt"),
            tokenizer=self.tokenizer,
            sequence_len=sequence_len,
            token_style=self.token_style,
        )
        val_set = Dataset(
            os.path.join(self.args.dataset, "dev.txt"),
            tokenizer=self.tokenizer,
            sequence_len=sequence_len,
            token_style=self.token_style,
        )
        test_set_1 = Dataset(
            os.path.join(self.args.dataset, "test.txt"),
            tokenizer=self.tokenizer,
            sequence_len=sequence_len,
            token_style=self.token_style,
        )
        ...
        self.train_loader = torch.utils.data.DataLoader(
            train_set, **train_data_loader_params
        )
        self.val_loader = torch.utils.data.DataLoader(
            val_set, **test_data_loader_params
        )
        self.test_loaders = [
            torch.utils.data.DataLoader(x, **test_data_loader_params) for x in test_set
        ]
        ...
        self.entity_extraction = EntityRecognition(
            self.args.pretrained_model,
        ...
        self.optimizer = torch.optim.Adam(
            self.entity_extraction.parameters(),
            lr=self.args.lr,
    ...
    def validate(self, data_loader):
        ...
        with torch.no_grad():
            for x, y, att in tqdm(data_loader, desc="eval"):
                ...
                correct += (
                    torch.sum(att * (y_predict == y).long()).item() - 2 * x.shape[0]
                )
                ...
                for ii in range(y.shape[0]):
                    for jj in range(len(y[ii])):
                        if x[ii][jj].item() == TOKEN_IDX[self.token_style]["PAD"]:
                            break
                        pred_str += (
                            self.tokenizer.convert_ids_to_tokens(x[ii][jj].item())
                            + "\t"
                        )
                        pred_str += str(reverse_entity_mapping[y[ii][jj].item()]) + "\t"
                        pred_str += str(
                            reverse_entity_mapping[
                                torch.argmax(y_predict[ii][jj]).item()
                            ]
                        )
                        pred_str += "\n"
                    pred_str += "[SENT_BREAK]\n"
    ...
    def train(self):
        # TODO I should refactor it as the finetune_sentence_classificaton_run() method
            ...
            for x, y, att in tqdm(self.train_loader, desc="train"):
                ...
                if self.args.gradient_clip > 0:
                    torch.nn.utils.clip_grad_norm_(
                        self.entity_extraction.parameters(), self.args.gradient_clip
                    )
                self.optimizer.step()
