...
def parse_entity_recognition_arguments():
    ...
    parser.add_argument(
        "--name", default="entity_recognition", type=str, help="name of run"
    )
    parser.add_argument(
        "--cuda",
        default=True,
    ...
    parser.add_argument(
        "--dataset",
        default="data/entity_extraction",
        type=str,
    ...
    parser.add_argument(
        "--decay", default=0, type=float, help="weight decay (default: 0)"
    )
    parser.add_argument(
        "--gradient-clip",
        default=-1,
        type=float,
        help="gradient clipping (default: -1 i.e., none)",
    )
    parser.add_argument(
        "--batch-size", default=32, type=int, help="batch size (default: 32)"
    )
    parser.add_argument(
        "--epoch", default=30, type=int, help="total epochs (default: 30)"
    )
    parser.add_argument(
        "--save-path",
        default="out/entity_extraction",
        type=str,
        help="model and log save directory",
    )
...
def parse_bert_sentence_classification_arguments():
    """
    This function is designed to parse command-line arguments specifically for
    BERT sentence classification tasks. The parsed arguments are stored in the variable.

    Returns:
    Namespace: variable containing all the parameters defined in the parser, accesible using the dot operator
    """
    ...
    parser.add_argument(
        "--dataset",
        default="data/sentence_classification",
        type=str,
        help="data directory containing train/dev/test files",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--epoch", default=10, type=int, help="total epochs (default: 200)"
    )
    parser.add_argument(
        "--fine-tune",
        default=True,
    ...
    parser.add_argument(
        "--save-path",
        default="out/sentence_classification",
        type=str,
        help="output log/result directory",
    )
    args = parser.parse_args()
    return args
...
def parse_inference_arguments():
    ...
    parser.add_argument(
        "--entity-extraction-weight",
        default="out/entity_extraction/weights.pt",
        type=str,
        help="saved weight for entity extraction model",
    )
    parser.add_argument(
        "--sentence-classification-weight",
        default="out/sentence_classification/weights.pt",
        type=str,
        help="saved weight for sentence classification model",
    )
    parser.add_argument(
        "--input-doc", default="samples/1.txt", type=str, help="input document path"
    )
    parser.add_argument(
        "--save-path",
        default="samples/1_predict.txt",
        type=str,
...
def parse_word_embedding_sentence_classification_args():
    parser = argparse.ArgumentParser(
        description="Sentence classification using word embedding"
    )
    parser.add_argument("--task", default="atk-pattern", type=str, help="Task name")
    parser.add_argument(
        "--w2v-file", default=None, type=str, help="word embedding file"
    )
    parser.add_argument(
        "--cuda",
        default=True,
