...
val_set = Dataset(
    os.path.join(args.dataset, "dev.txt"),
    tokenizer=tokenizer,
    sequence_len=sequence_len,
    token_style=token_style,
)
...
entity_extraction = EntityRecognition(
...
optimizer = torch.optim.Adam(
    entity_extraction.parameters(), lr=args.lr, weight_decay=args.decay
)
        ...
        for x, y, att in tqdm(data_loader, desc="test"):
            ...
            for ii in range(y.shape[0]):
                for jj in range(len(y[ii])):
                    ...
                    pred_str += str(
                        reverse_entity_mapping[torch.argmax(y_predict[ii][jj]).item()]
                    )
                    pred_str += "\n"
                pred_str += "[SENT_BREAK]\n"
        ...
        for x, y, att in tqdm(train_loader, desc="train"):
            ...
            if args.gradient_clip > 0:
                torch.nn.utils.clip_grad_norm_(
                    entity_extraction.parameters(), args.gradient_clip
                )
            optimizer.step()
