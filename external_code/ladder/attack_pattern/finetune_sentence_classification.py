...
def hypertune(f: TextIO, rf: TextIO):
    """
    Fine tunning of a sentence classification model, used to predict if a sentence contains an attack pattern or not.

    Parameters:
    f (TextIO): file containing the parameters given to the finetunning script
    rf (TextIO): report file containing the output of the training statistics and metrics.
    ...
    keys, values = zip(
        *params_dict.items()
    )  # the * unpacks the set of returned items into separate arguments
    permutations_dicts = [
        dict(zip(keys, v)) for v in itertools.product(*values)
    ]  # itertools.product(*values) = [('bert-base-uncased', 0.000005, 16, 20, 0.5, True)]
    ...
    rf.write(
        "counter,seed,lr,batch_size,epoch,dropout,fine_tune,test_acc,f1,test_acc_avg,f1_avg,save_path\n"
    )
    for oneConfig in permutations_dicts:
        ...
        for seed in range(1, 2):
            ...
            arg.__setattr__(
                "seed", seed
            )  # using __setattr__ When the attribute name is not known until runtime.
            for oneParamKey, oneParamValue in oneConfig.items():
                # basically getting the params fromt the params_dict
