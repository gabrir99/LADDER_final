...
from models import (
    EntityRecognition,
    SentenceClassificationBERT,
    SentenceClassificationRoBERTa,
)
from config import *
...
def infer():
    ...
    entity_model.load_state_dict(
        torch.load(args.entity_extraction_weight, map_location=device)
    )

    if MODELS[args.sentence_classification_model][3] == "bert":
        sentence_model = SentenceClassificationBERT(
            args.sentence_classification_model, num_class=2
        ).to(device)
        sentence_model.load_state_dict(
            torch.load(args.sentence_classification_weight, map_location=device)
        )
    elif MODELS[args.sentence_classification_model][3] == "roberta":
        sentence_model = SentenceClassificationRoBERTa(
            args.sentence_classification_model, num_class=2
        ).to(device)
        sentence_model.load_state_dict(
            torch.load(args.sentence_classification_weight, map_location=device)
