...
# specifies which classes, functions, or variables should be imported when a user imports the module using from module import *.
...
class TrainTransformersNER:
    ...
    def __setup_model_data(self, dataset, lower_case):
        ...
        if self.model is not None:
            return
        if (
            self.args.is_trained
        ):  # True if there in the checkpoint_dir there are the parameters download from huggingface
    ...
    def __setup_loader(self, data_type: str, batch_size: int, max_seq_length: int):
        """
        Create the data loader by trasforming the dataset in input using the self.transforms class
    ...
    def train(
        ...
        self.__setup_model_data(
            self.args.dataset, self.args.lower_case
        ) 
