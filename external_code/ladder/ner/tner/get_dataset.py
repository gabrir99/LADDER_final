...
def get_dataset_ner(
    ...
    """
    assert (
        data_names or custom_data_path
    ), "either `data_names` or `custom_data_path` should be not None"
    ...
    for (
        ...
        unseen_entity_set = (
            ues if len(unseen_entity_set) == 0 else unseen_entity_set.intersection(ues)
        )
    if (
        len(data) > 1
...
def get_dataset_ner_single(
    ...
    created to handle different dataset format, based on the names of the dataset.
    ...
    if data_name in ["ontonotes5", "conll2003"]:
        ...
        if not os.path.exists(data_path):
            ...
            for i in ["train", "dev", "test"]:
                if data_name == "ontonotes5":
                    ...
                    conll_formatting(
                        file_token="{}/data/conll2003/conll2003-{}.words".format(
                            data_path, i
                        ),
                        file_tag="{}/data/conll2003/conll2003-{}.nertags".format(
                            data_path, i
                        ),
                        output_file="{}/{}.txt".format(data_path, i),
                    )
            ...
            for e in entities:
                ...
                _tokens_tmp = list(
                    filter(
                        ...
                        map(
                            lambda m: m.replace(" ", ""),
                            re.split(r"\b", text[last_end:start]),
                        ),
                    )
                )
                last_end = end
                ...
                _tags += ["B-{}".format(entity_type)] + ["I-{}".format(entity_type)] * (
                    len(_mention_token) - 1
                )
        ...
        def convert_to_iob(path, export):
            ...
            with open(path, "r") as f:
                ...
                conll_formatting(
                    tokens=tokens,
                    tags=tags,
                    output_file=os.path.join(data_path, export),
                    sentence_division=".",
                )
        ...
        if not os.path.exists(data_path):
            ...
            tar = tarfile.open(
                "{0}/panx_dataset/{1}.tar.gz".format(cache_dir, panx_la), "r:gz"
            )
            tar.extractall(data_path)
            tar.close()
            for v in files_info.values():
                os.system(
                    "sed -e 's/{0}://g' {1}/{2} > {1}/{2}.txt".format(
                        panx_la, data_path, v.replace(".txt", "")
                    )
                )
...
def decode_all_files(
    ...
    for (
        name,
        filepath,
    ) in files.items():  # creates the dictionary containing test, train, val datasets
