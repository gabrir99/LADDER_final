...
class SetExpander(EntityExtraction):
    ...
    def get_entities(self, text):
        ...
        self.expander = SetExpand(
            np2vec_model_file=self.config["model"],
            grouping=grouping,
        ...
        for iter in range(iterations):
            expanded_terms_list = self.expander.expand(
                expanded_terms_set, topn=topn, threshold=threshold
            )
            if (
                len(expanded_terms_set) < maxTermsCount
            ) and expanded_terms_list is not None:
                for term, similarity in expanded_terms_list:
                    if len(expanded_terms_set) == maxTermsCount:
                        break
                    expanded_entities_list.append(
                        Entity(-1, -1, term, "Expanded term", similarity)
                    )
