...
class TransformersNER(EntityExtraction):
    """
    Class extracting the configs from the cfg dictionary in order
    to intialize the parameters with default values if missing
    """
    ...
    def train(self):
        """
        Initialize the parameters using the config dictionary and then calls the
        tner trainer to train the model
        """
        config = self.config
