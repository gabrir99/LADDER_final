...
class BertForSequenceClassificationUserDefined(BertPreTrainedModel):
    def __init__(self, config):
        """
        It is a bert mdoel with on top:
        - dropout layer,
        - 2 linear layers one on top of the other
        """
        super().__init__(config)
        self.num_labels = config.num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        # TODO this 2 linear layers are equivalent to a single linear layer?
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                for i in range(len(w)):
                    # crossentropy with a weighted sum using the weights w and exponentiation for weight scaling.
                    loss += math.exp(w[i] - 1) * loss_fct(
...
class RelationClassificationBERT(BertForSequenceClassificationUserDefined):
    def __init__(self, config):
        super().__init__(config)
    def from_pretrained_custom(self, path : str):
...
class RobertaForSequenceClassificationUserDefined(BertPreTrainedModel):
    ...
    def __init__(self, config):
        """
        It is a roberta mdoel with on top:
        - dropout layer,
        - 2 linear layers one on top of the other
        """
        print("Initing")
